				...
				<div class="main-table-wapper">
					<table class="table main-table">
						...
						<tr class="body-row" v-if="list.length == 0">
							<td colspan="15" style="text-align: center; height: 100px;line-height: 100px;">{{noDataMessage}}</td>
						</tr>
						<template v-for="item in list">
							<tr :class="{actived: item.isActived, 'body-row': true}">
								...
								<td><span>搜索</span></td>
								<td>
									<span v-if="!item.dmpData[0].search || item.dmpData[0].search.searchMaxBid == null">--</span>
									<span v-if="item.dmpData[0].search && item.dmpData[0].search.searchMaxBid != null">{{(item.dmpData[0].search.searchMinBid / 100).toFixed(2)}}~{{(item.dmpData[0].search.searchMaxBid / 100).toFixed(2)}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.impression != null ? item.dmpData[0].search.impression : "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.click != null ? item.dmpData[0].search.click :  "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.ctr != null ? item.dmpData[0].search.ctr :  "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.cashFee != null ? item.dmpData[0].search.cashFee/100 :  "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.virtualFee != null ? item.dmpData[0].search.virtualFee/100 :  "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.clickUnitPrice != null ? item.dmpData[0].search.clickUnitPrice :  "--"}}</span>
								</td>
								<td><span>{{item.dmpData[0].search && item.dmpData[0].search.costPerMills != null ? item.dmpData[0].search.costPerMills :  "--"}}</span>
								</td>
								<td rowspan="2"><span>
								<b class="operat-link">
								  <a href="" @click.prevent="summary.stateCode == 3 ? viewFlight(item.flightId) : modify(item.flightId)">{{summary.stateCode == 3 ? '查看' : '修改'}}</a><a href="" :class="{'action-disabled': summary.stateCode == 3}" @click.prevent="copy(item.flightId)">复制单元</a><a
									href="" :class="{'action-disabled': summary.stateCode == 3}" @click.prevent="newIdea(item.flightId, item.type)">新建创意</a>
								</b>
                  				</span></td>
							</tr>
							<tr>
								<td><span>推荐</span></td>
								<td><span>{{item.dmpData[0].recomment.adBid == null ? "--" : (Number(item.dmpData[0].recomment.adBid) / 100).toFixed(2)}}</span>
								</td>
								<td><span>{{item.dmpData[0].recomment.impression == null ? "--" : item.dmpData[0].recomment.impression}}</span>
								</td>
								<td><span>{{item.dmpData[0].recomment.click == null ? "--" : item.dmpData[0].recomment.click}}</span>
								</td>
								<td><span>{{item.dmpData[0].recomment.ctr == null ? "--" : item.dmpData[0].recomment.ctr}}</span>
								</td>
								<td><span>{{item.dmpData[0].recomment.cashFee == null ? "--" : item.dmpData[0].recomment.cashFee/100}}</span>
								</td>
								<td><span>{{item.dmpData[0].recomment.cashFee == null ? "--" : item.dmpData[0].recomment.virtualFee/100}}</span>
								</td>
...
<script>
	...
	export default {
		...
		created(){
			...
			this.summary.state = this.$route.params.state;
			this.getSummary();
			this.getList();
		},
		mounted() {
						...
						if (res.data.code == 200) {
							...
							http.get('/api/report/flight/consume',{
								params:{
									...
									startTime: so.startTime?+so.startTime:0,
									endTime: so.endTime?+so.endTime:0,
									productLine: 3  //1-合约 2-众达 3-点效 4-站外
								}
							}).then((res) => {
								let campaignCounts = res.data.data;
								that.list.forEach((item) => {
									let campaignRecomment = campaignCounts.find(campaignCount => {
										return item.dspFlightId == campaignCount.dspFlightId && campaignCount.productType == 2;
									});
									let campaignSearch = campaignCounts.find(campaignCount => {
										return item.dspFlightId == campaignCount.dspFlightId  && campaignCount.productType == 1;
									});
									if (campaignRecomment) {
										Vue.set(item.dmpData[0].recomment, "impression", campaignRecomment['impression']);
					                    Vue.set(item.dmpData[0].recomment, "click", campaignRecomment['click']);
					                    Vue.set(item.dmpData[0].recomment, "ctr", campaignRecomment['ctr']);
					                    Vue.set(item.dmpData[0].recomment, "cashFee", campaignRecomment['cashFee']);
					                    Vue.set(item.dmpData[0].recomment, "virtualFee", campaignRecomment['virtualFee']);
					                    Vue.set(item.dmpData[0].recomment, "clickUnitPrice", campaignRecomment['clickUnitPrice']);
					                    Vue.set(item.dmpData[0].recomment, "costPerMills", campaignRecomment['costPerMills']);					               
									} else {
										Vue.set(item.dmpData[0].recomment, "impression", null);
					                    Vue.set(item.dmpData[0].recomment, "click", null);
					                    Vue.set(item.dmpData[0].recomment, "ctr", null);
					                    Vue.set(item.dmpData[0].recomment, "cashFee", null);
					                    Vue.set(item.dmpData[0].recomment, "virtualFee", null);
					                    Vue.set(item.dmpData[0].recomment, "clickUnitPrice", null);
					                    Vue.set(item.dmpData[0].recomment, "costPerMills", null);
									}
									if (campaignSearch) {
										Vue.set(item.dmpData[0].search, "impression", campaignSearch['impression']);
					                    Vue.set(item.dmpData[0].search, "click", campaignSearch['click']);
					                    Vue.set(item.dmpData[0].search, "ctr", campaignSearch['ctr']);
					                    Vue.set(item.dmpData[0].search, "cashFee", campaignSearch['cashFee']);
					                    Vue.set(item.dmpData[0].search, "virtualFee", campaignSearch['virtualFee']);
					                    Vue.set(item.dmpData[0].search, "clickUnitPrice", campaignSearch['clickUnitPrice']);
					                    Vue.set(item.dmpData[0].search, "costPerMills", campaignSearch['costPerMills']);
									} else {
										Vue.set(item.dmpData[0].search, "impression", null);
					                    Vue.set(item.dmpData[0].search, "click", null);
					                    Vue.set(item.dmpData[0].search, "ctr", null);
					                    Vue.set(item.dmpData[0].search, "cashFee", null);
					                    Vue.set(item.dmpData[0].search, "virtualFee", null);
					                    Vue.set(item.dmpData[0].search, "clickUnitPrice", null);
					                    Vue.set(item.dmpData[0].search, "costPerMills", null);
									}
								})
							}).catch((error) => {
								console.log(error);
