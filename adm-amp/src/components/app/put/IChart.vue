<template>
	<div class="index-map">
		<h2 class="map-title"><span>整体情况</span><i>（以下数据有近三十分钟的统计延迟）</i>
			<span class="right-action" @click="showDataMap"><em class="icon" :class="{'icon-arrow-up': isDataMapShow, 'icon-arrow-down': !isDataMapShow}"></em>{{isDataMapShow ? '收起' : '展开'}}</span>
		</h2>
		<div class="map-data">
			<ul>
				<li>
					<strong>曝光量（次）<em :title="productLine==2?'国美众达-所有计划在选定时间内的点击数。':'国美点效-所有计划在选定时间内的点击数。'" class="icon icon-help"></em></strong>
					<span>{{ overAll.impression }}</span>
				</li>
				<li>
					<strong>点击量（次）<em :title="productLine==2?'国美众达-所有计划在选定时间内的展示次数。':'国美点效-所有计划在选定时间内的展示次数。'" class="icon icon-help"></em></strong>
					<span>{{ overAll.click }}</span>
				</li>
				<li>
					<strong>平均点击率<em title="平均点击率=点击量/曝光量*100%" class="icon icon-help"></em></strong>
					<span>{{ ctr }}%</span>
				</li>
				<li>
					<strong>广告花费（元）<em :title="productLine==2?'在选定时间内，国美众达产品线广告花费之和，包括现金、虚拟金。':'在选定时间内，国美点效产品线广告花费之和，包括现金、虚拟金。'" class="icon icon-help"></em></strong>
					<span>{{ overAll.adCost }}</span>
				</li>
				<li v-if="productLine === 2">
					<strong>返利花费（元）<em title="在选定时间内，国美众达产品线下返利账户的总花费。" class="icon icon-help"></em></strong>
					<span>{{ overAll.rebateCost }}</span>
				</li>
			</ul>
		</div>
		<transition name="zoom-in-top">
			<div v-show="isDataMapShow">
				...
				<div class="map-show">
					<i-chart :lineChartOption="lineChartOption" :isDataMapShow="isDataMapShow"></i-chart>
				</div>
			</div>
		</transition>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			showDataMap() {
				this.isDataMapShow = !this.isDataMapShow;
			},
			loadReport() {
				let vm = this;
...
<style scoped>
	...
	.map-title .right-action{display: inline-block;float: right;font-size: 16px;color: #23272c;cursor: pointer;}
	.map-title .right-action .icon {font-size: 12px; margin-right: 5px;}
