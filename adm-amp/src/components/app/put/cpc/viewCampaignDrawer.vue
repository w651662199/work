				...
				<div class="form-column">
					<el-form :model="formData" ref="formData" label-width="140px">
						...
						<el-form-item label="返利活动：">
							<span>{{formData.isRebate == 1 ? '返利活动' : '非返利活动'}}</span>
						</el-form-item>
						<el-form-item label="返利日预算：" prop="dailyRebateBudget" v-show="formData.isRebate" class="daliyRebate">
							<span v-if="formData.rebateLimited == 0">不限</span>
							<span v-else>{{formData.dailyRebateBudget}}元/天</span>
						</el-form-item>
					</el-form>
				</div>
			</div>
