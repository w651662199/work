          ...
          <table class="table main-table">
            <thead>
              <tr :class="{actived: isActived, 'list-header': true}">
                ...
                <th class="w140"><span>返利日预算<em class="icon icon-TableHead-help" :title="rebateDayBudget"></em></span></th>
            ...
            <tbody>
              ...
              <template v-for="item in list">
                <tr :class="{actived: item.isActived, 'body-row': true}">
                  ...
                  <!-- 状态 -->
                  <td><span :style="{'color': isSystemPause(item.state) ? '#d30312' : '#23272c'}">
                    {{getCampaignState(item.state)}}
                  <sup v-if="isSystemPause(item.state)" :title="getSystemPauseReason(item.state)"><em class="icon icon-help"></em></sup></span>
                  </td>
                  <!-- 有效期 -->
                  <td>
...
export default {
  ...
  data() {
    return {
      ...
      rebateCost:'该投放计划在选定时间内的返利总现金花费',
      page: {
        totalCount: 0,
        currentPage: 1,
        pageSizes: [20, 30, 50],
  ...
  methods: {
        ...
        if(res.data.code == 200){
          ...
          if (that.list.length === 0) {
            ...
            for(var item of that.list){
	          ...
	          //返利总花费接口
	          http.get("/api/account/expenses/campaign",{
		              params:{
			              dspCampaignIds:dspCampaignIds.join(','),
			              startTime: +so.startTime,
			              endTime: +so.endTime
	              ...
	              .then((res) => {
		              if(res.data.code == 200){
			              ...
			              that.list.forEach((item) => {
				              let campaign = campaignCounts.find(campaignCount => {
					              return item.dspCampaignId == campaignCount.dspCampaignId;
				              });
				              if (campaign) {
    ...
    campaignStartHandler() {
      ...
      http.get('/api/pline/account/all/info?productLine=2').then((res)=> {
    ...
    modify(id, item){
      this.getData(id, (data) => {
        let campaignData = {
          ...
          tempDailyRebateBudget: item.dailyRebateBudget,
          tempAdLimited: item.adLimited,
          tempRebateLimited: item.rebateLimited,
          tempIsRebate: item.isRebate
        };
      	actions.controlDrawer(this.$store, {
      		show: true,
      		action: 'modify',
