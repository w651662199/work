...
const getTableHead = (mainContainer, tableElement, isLeftFixed, hasSelectHandler, leftFloatNumber) => {
	...
	tableHeadDivCon = domCreator('div', null, {
		class: 'table__fixed-head--wapper fixed-header'
	});
	var tableHeadTableCon = domCreator('table', null, {
		class: 'table fixedtablehead'
	});
...
const getfixColumnTable = (mainContainer, tableElement, hasSelectHandler, leftFloatNumber) => {
	...
	tableColumnDivCon = domCreator('div', null, {
		class: 'table__fixed-columns--wapper'
	});
...
const getScrollContainer = (mainContainer, tableElement) => {
	...
	if (judgeBarPosition(mainContainer) == 1) {
		var scrollContainerDiv = domCreator('div', null, {
			class: 'table__fixed-scrollbar--wapper'
		});
	} else {
		var scrollContainerDiv = domCreator('div', null, {
			class: 'table__static-scrollbar--wapper'
		});
	}
	scrollContainerDiv.style.width = mainContainer.clientWidth + 'px';
	scrollContainerDiv.style.left = mainContainer.offsetLeft + 'px';

	var scrollContentDiv = domCreator('div', null, {
		class: 'table__fixed-scrollbar--content'
	});
...
export const initWindowResize = (containerId, hasSelectHandler, leftFloatNumber) => {
	on(window, 'resize', () => {
		tableHandler(containerId, hasSelectHandler, leftFloatNumber);
	});
};
