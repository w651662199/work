...
class HighRebateTask {
	...
	*/
	...
	public function run() {
		...
		// 新增用户数
		$incrUserCount = $this->getIncrUser(date("Y-m-d"));
	...
	public function runYest() {
		...
		//活动当天收集到的手机号数
		$incrUserCount = $this->getIncrUser(date("Y-m-d", strtotime("-1 day")));

		$this->_successLog("02 incr user count incr=" . $incrUserCount);

		$appIncrCount = $this->getIncrUser();
	...
	// 获取新增用户数
	private function getIncrUser($date) {

		if ($date == null) {
			$gomeUserSql = "SELECT mobile FROM ad_gome_user WHERE create_time>='2018-08-29 00:00:00'";
			$newUserSql = "SELECT mobile FROM mm_new_user WHERE create_time>='2018-08-29 00:00:00'";
		} else {
			$start = $date . " 00:00:00";
			$end = $date . " 23:59:59";
			$gomeUserSql = "SELECT mobile FROM ad_gome_user WHERE create_time>='". $start ."' AND create_time<='" . $end . "'";
			$newUserSql = "SELECT mobile FROM mm_new_user WHERE create_time>='". $start ."' AND create_time<='" . $end . "'";
		}

		$gomeUsers = $this->_getData($gomeUserSql);
		$newUsers = $this->_getData($newUserSql);

		$users = array();
		foreach ($gomeUsers as $key => $value) {
			$users[] = $value['mobile'];
		}
		$nusers = array();
		foreach ($newUsers as $key => $value) {
			$nusers[$value['mobile']] = 1;
		}
		$count = 0;
		foreach ($users as $mobile) {
			if (isset($nusers[$mobile])) {
				$count++;
			}
		}
		$otherCount = count($users) - $count;

		$this->_successLog("incr user count incr=" . $count . " other count=" + $otherCount);
