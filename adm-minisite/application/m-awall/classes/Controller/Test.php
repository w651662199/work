...
class Controller_Test extends Controller_Render {
	...
	public function action_index() {
		...
		$insertResult = Business::factory('Test_Imp')->create($data);

		Log::instance()->add(Log::NOTICE, '页面曝光入库结果：' . json_encode($insertResult));

		$this->_layout->content = View::factory('test/index')
	...
	public function action_getcode() {
		...
		if (preg_match("/^1\d{10}$/", $mobile)) {
			try {
				$cacheCountKey = "TEST_WAP_CODE_C_" . $mobile;
				$isSend = Redisd::instance('awall')->get($cacheCountKey);
				if ($isSend != FALSE) {
					$this->failed('请不要重复获取');
				} else {
					$cacheKey = "TEST_WAP_CODE_" . $mobile;
	...
	public function action_check() {
		...
		if (preg_match("/^1\d{10}$/", $mobile)) {
			$mobileKey = "TEST_WAP_PINGAN_MOBILE_" . $mobile;
	...
	public function action_register() {
		...
		try {
			$cacheKey = "TEST_WAP_CODE_" . $mobile;
			$perDateKey = "TEST_WAP_PINGAN_" . date("Y-m-d");
			$mobileKey = "TEST_WAP_PINGAN_MOBILE_" . $mobile;
			$wechatKey = "TEST_WAP_PINGAN_WECHAT_" . $wechatId;
			$totalKey = "TEST_WAP_PINGAN_TOTAL_COUNT";
			...
			if ($redisVerifyCode != FALSE && $redisVerifyCode == $verifyCode) {
				...
				try {
					$insertResult = Business::factory('Test_Pingan')->create($data);
		...
		if ($wechatId != '') {
			try {
				...
				if ($wechatId != false || $perDayCount >= 1000 || $totalCount >= 5000 || $isExpire) {
					...
					$data['landingPage'] = $smsConfig['testLandingPage'];
				}
