	...
	<div>
		...
		<div class="query-information">
			<div class="information-fn">
				<div class="fn-item">
					<span>推广组：</span>
					<el-select v-model="searchParams.groupId" style="width:130px;display:inline-block;" placeholder="全部">
					...
					<table class="table main-table">
						...
						<tbody>
							<tr v-for="item in list">
								<td>
									<span v-if="isDaily"><router-link :to="{name: 'groupHourReport', params: {slotId: item.slotId, groupId: item.groupId, startDate: item.adDate, endDate: item.adDate}}">{{item.adDate}}</router-link></span>
									<span v-if="isSummary"><router-link :to="{name: 'groupHourReport', params: {slotId: item.slotId, groupId: item.groupId, startDate: item.adStartDate, endDate: item.adEndDate}}">{{item.adStartDate}}<br>{{item.adEndDate}}</router-link></span>
...
export default {
	...
	methods: {
		getList() {
			...
			let url = this.isDailyReport ? '/api/report/groups/daily' : '/api/report/groups/summary';
			Http.get(url, {
				params: {
					page: this.page.currentPage,
		...
		initSearchOptions(search) {
			...
			this.medias = medias;
			this.slots = slots;
			this.setAdpMapData(search);
			this.setAdmMapData(search);
			this.isInit = false;
		},
		changeTab(tab) {
