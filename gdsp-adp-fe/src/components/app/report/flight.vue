					...
					<table class="table main-table">
						...
						<tbody>
							<tr v-for="item in list">
								<td>
									<span v-if="isDaily"><router-link :to="{name: 'flightHourReport', params: {slotId: item.slotId, flightId: item.flightId, startDate: item.adDate, endDate: item.adDate}}">{{item.adDate}}</router-link></span>
									<span v-if="isSummary"><router-link :to="{name: 'flightHourReport', params: {slotId: item.slotId, flightId: item.flightId, startDate: item.adStartDate, endDate: item.adEndDate}}">{{item.adStartDate}}<br>{{item.adEndDate}}</router-link></span>
...
export default {
	...
	methods: {
		...
		getRtList() {
			this.setUrlParams();
			Http.get('/api/report/flights/rt', {
				params: {
		...
		getList() {
			...
			let url = this.isDailyReport ? '/api/report/flights/daily' : '/api/report/flights/summary';
			Http.get(url, {
				params: {
					page: this.page.currentPage,
					number: this.page.pageSize,
