	...
	<div>
		...
		<div class="query-information">
			<div class="information-fn">
				...
				<div class="fn-item">
					<span>计划：</span>
					<el-select v-model="searchParams.campaignId" style="width:130px;display:inline-block;" placeholder="全部" @change="campaignChange">
						<el-option v-for="campaign of campaignOptions" :label="campaign.name" :value="campaign.campaignId"></el-option>
					</el-select>
				</div>
				<div class="fn-item">
					<span>单元：</span>
					<el-select v-model="searchParams.flightId" style="width:130px;display:inline-block;" placeholder="全部">
						<el-option v-for="flight of flightOptions" :label="flight.name" :value="flight.flightId"></el-option>
					</el-select>
				</div>
				<div class="fn-item">
					<span>创意：</span>
					<el-select v-model="searchParams.materialId" style="width:130px;display:inline-block;" placeholder="全部">
						<el-option v-for="material of materialOptions" :label="material.name" :value="material.materialId"></el-option>
					</el-select>
				</div>
			</div>
			<div class="information-fn">
					...
					<table class="table main-table">
						<thead>
							<tr>
								...
								<th class="w80"><span>单元</span></th>
								<th class="w80"><span>创意</span></th>
						...
						<tbody>
							<tr v-for="item in list">
								<td>
									<span v-if="isDaily"><router-link :to="{name: 'materialHourReport', params: {slotId: item.slotId, campaignId: item.campaignId, flightId: item.flightId, materialId: item.materialId, startDate: item.adDate, endDate: item.adDate}}">{{item.adDate}}</router-link></span>
									<span v-if="isSummary"><router-link :to="{name: 'materialHourReport', params: {slotId: item.slotId, campaignId: item.campaignId, flightId: item.flightId, materialId: item.materialId, startDate: item.adStartDate, endDate: item.adEndDate}}">{{item.adStartDate}}<br>{{item.adEndDate}}</router-link></span>
								...
								<td><span :title="item.flightName"><i class="ellipsis">{{item.flightName}}</i></span></td>
								<td><span :title="item.materialName"><i class="ellipsis">{{item.materialName}}</i></span></td>
...
export default {
	...
	created() {
		...
		if (queryObj) {
			...
			this.searchParams.flightId = +queryObj.flightId;
			this.searchParams.materialId = +queryObj.materialId;
	...
	methods: {
		changeTab(tab) {
			...
			this.searchDateRange = [moment().subtract(1, 'days').startOf('day').valueOf(), moment().subtract(1, 'days').startOf('day').valueOf()];
			this.isDailyReport = true;
			this.search();
		},
		getRtList() {
			this.setUrlParams();
			Http.get('/api/report/materials/rt', {
				params: {
					...
					flightId: this.searchParams.flightId,
		...
		getList() {
			...
			let url = this.isDailyReport ? '/api/report/materials/daily' : '/api/report/materials/summary';
			Http.get(url, {
				params: {
					page: this.page.currentPage,
					number: this.page.pageSize,
		...
		initSearchOptions(search) {
			...
			let flights = this.mapToList(search.flights);
			let materials = this.mapToList(search.materials);
			...
			this.flightOptions = [...this.flightOptions, ...flights];
		...
		biddingModeChange() {
			...
			this.campaignOptions = campaigns;
			if (this.searchParams.campaignId !== 0) {
		...
		campaignChange() {
			...
			if (this.searchParams.campaignId === 0) {
				let campaignIds = [];
				for (let c of this.campaignOptions) {
					campaignIds.push(c.campaignId);
				}
				for (let f of this.flights) {
					if (campaignIds.indexOf(f.campaignId) > -1) {
						flights.push(f);
					}
				}
			} else {
				for (let f of this.flights) {
					if (f.campaignId === this.searchParams.campaignId) {
						flights.push(f);
					}
				}
			}
			this.flightOptions = flights;
			this.searchParams.flightId = 0;
		},
		flightChange() {
			let materials = [{
				name: '全部',
			...
			if (this.searchParams.flightId === 0) {
				let flightIds = [];
				for (let f of this.flightOptions) {
					flightIds.push(f.flightId);
				}
				for (let m of this.materials) {
					if (flightIds.indexOf(m.flightId) > -1) {
						materials.push(m);
					}
				}
			} else {
				for (let m of this.materials) {
					if (m.flightId === this.searchParams.flightId) {
						materials.push(m);
					}
				}
			}
			this.materialOptions = materials;
			this.searchParams.materialId = 0;
		},
		publisherChange() {
			let medias = [{
				name: '全部',
